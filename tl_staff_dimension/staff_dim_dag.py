...
def create_table():
    ...
    CREATE TABLE IF NOT EXISTS staff_dimension (
        ...
        staff_state VARCHAR(255) NOT NULL,
...
def process_and_load_html():
    # Read the cleaned staff data from HTML
    ...
    merged_file_path = f"{dir}/staff_dim.csv"
    staff_data_df.to_csv(merged_file_path, index=False)

    # Load data into database
    engine = create_engine(db_url)
    staff_data_df.to_sql('staff_dimension', engine, if_exists='append', index=False)
